...
def userPrompt():
  try:
    ...
    query = f"{refined_prompt}. provide only code, no explanation language or any additional guide or information. Just code"
...
def get_class_diagrams():
  try:
    ...
    print(mermaid_input) 
    mermaid_prompt = f"Generate a Mermaid.js mindmap only using the code given: \n {mermaid_input}. return only the mermaid code, no explanation or anything else"
